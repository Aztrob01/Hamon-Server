...
def main():
    ...
    def run_flask():
                ...
                while True:
                    ...
                    with open("./data.json", "r") as data:
                        ...
                        return{
                            ...
                            "cpu_last_update": cpu_timestamp,
                            "cpu_catch_interval": f"{cpu_interval}ms",
                            "cores": cores,
                            "threads": threads,
                            ...
                            "hardware_mem": memory_t,
                            "mem_usage": f"{memory.free / (1024 ** 3):.1f}",
                            "mem_percent": f"{memory.percent}%",
                            "virtual_mem": swap_t,
                            "virtual_usage": f"{swap.free / (1024 ** 3):.1f}",
                            "mem_absolute": mem_all,
        ...
        def lowlevel():
            #? psutil CPU catch
            while True:
                cpu_percent = psutil.cpu_percent(interval=mainInterval)
                return {
                    "cpu_per": f"{cpu_percent}%",
                    "cpu_upd": "None",
                    "cpu_int": f"{mainInterval}s",
                    "cores": cores,
                    "threads": threads,
                    "program_interval": f"{mainInterval}s",
        ...
        def index():
            match selectMode:
                case 1:
                    return render_template('highlevel.html')
                case 2:
                    return render_template('lowlevel.html')
                case _:
        ...
        def data():
            match selectMode:
                case 1:
                    return jsonify(lowlevel())
                case 2:
                    return jsonify(highlevel())
                case 3:
                    return jsonify(comparision())
